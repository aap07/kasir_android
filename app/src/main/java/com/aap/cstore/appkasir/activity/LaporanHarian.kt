...
import com.aap.cstore.appkasir.adapter.RclvLaporanHarian
...
class LaporanHarian : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setToolbar(this,"Laporan Harian")

        lapPenjualan()

    }

    private fun lapPenjualan(){
        val bulan = intent.getStringExtra("bulan")
        val bulanan = SugarRecord.listAll(Transaksi::class.java).filter { l -> l.status == false && l.tanggalTransaksi?.substring(3, l.tanggalTransaksi!!.indexOf(" ")).equals(bulan) }
        if(bulanan != null) {
            val transaksi = bulanan.distinctBy { it.tanggalTransaksi?.substring(0, it.tanggalTransaksi?.indexOf(" ")!!) }
            val sorted = transaksi.sortedBy { l -> l.tanggalTransaksi }.reversed()
            if (transaksi.isNotEmpty()) {
                tvEmpty.visibility = View.GONE
                textView12.visibility = View.VISIBLE
                textView12.setText("Laporan Harian")
                rclv.apply {
                    adapter = RclvLaporanHarian(
                        this@LaporanHarian,
                        sorted as MutableList<Transaksi>
                    )
                    layoutManager = LinearLayoutManager(this@LaporanHarian)
    ...
    fun createPDF(transaksi: Transaksi){
        ...
        val title = transaksi!!.tanggalTransaksi?.substring(0, transaksi!!.tanggalTransaksi?.indexOf(" ")!!)?.replace("/","_")
        val date = SimpleDateFormat("dd/MM/yyyy mm:hh:ss").parse(transaksi!!.tanggalTransaksi)
        val fileName = "LAPORAN_HARIAN_$title.pdf"
        val file = File(folder.absolutePath,fileName)
        val doc = PdfUtils(file.absolutePath)
        val tanggal = transaksi!!.tanggalTransaksi?.substring(0, transaksi!!.tanggalTransaksi?.indexOf(" ")!!)
        val iTransaksi = SugarRecord.listAll(Transaksi::class.java).filter { l -> l.status == false && l.tanggalTransaksi?.substring(0, l.tanggalTransaksi!!.indexOf(" ")).equals(tanggal) }
