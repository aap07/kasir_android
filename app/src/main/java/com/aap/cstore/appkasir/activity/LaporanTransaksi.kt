...
import com.aap.cstore.appkasir.adapter.RclvLaporanTransaksi
...
class LaporanTransaksi : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setToolbar(this,"Transaksi Harian")

        lapTrans()

    }

    private fun lapTrans(){
        val tanggal = intent.getStringExtra("tanggal")
        if(tanggal!=null){
            val listTransaksi = SugarRecord.listAll(Transaksi::class.java).filter { l -> l.tanggalTransaksi?.substring(0, l.tanggalTransaksi?.indexOf(" ")!!).equals(tanggal) }
//            val sorted = listTransaksi.sortedBy { l -> l.tanggalTrasaksi }.reversed()
            if (listTransaksi.isNotEmpty()){
                tvEmpty.visibility = View.GONE
                textView12.visibility = View.VISIBLE
                textView12.setText("History Transaksi")
                rclv.apply {
                    adapter = RclvLaporanTransaksi(this@LaporanTransaksi, listTransaksi as MutableList<Transaksi>)
                    layoutManager = LinearLayoutManager(this@LaporanTransaksi)
    ...
    fun createPDF(transaksi: Transaksi){
        ...
        if (id != null){
            val histori = SugarRecord.find(Transaksi::class.java,"id = ?",id).firstOrNull()
            if (histori != null) {
                val folder = File(Environment.getExternalStorageDirectory(),"Laporan Penjualan")
                if(!folder.exists()) folder.mkdir()
                val title = histori.tanggalTransaksi?.substring(0, histori.tanggalTransaksi?.indexOf(" ")!!)?.replace("/","_")
                val date = SimpleDateFormat("dd/MM/yyyy mm:hh:ss").parse(histori.tanggalTransaksi)
                val fileName = "LAPORAN_TRANSAKSI_$title.pdf"
                val file = File(folder.absolutePath,fileName)
                val doc = PdfUtils(file.absolutePath)
                val iTransaksi = SugarRecord.find(Transaksi::class.java,"id = ?",
                    histori.id.toString()
                )
                ...
                doc.addParagraf("History Transaksi", PdfUtils.fontTitle, PdfUtils.align_center)
                doc.addNewEnter()
                doc.addNewEnter()
